...
export class CandidateController {
  ...
  getAll = async (req: Request, res: Response): Promise<void> => {
    try {
      const filters = {
        ...
        enrollmentYear: req.query.enrollmentYear ? parseInt(req.query.enrollmentYear as string, 10) : undefined,
  ...
  create = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (existing) {
        res.status(409).json({
          success: false,
          error: 'Un candidat avec cet email existe deja'
        });
        return;
      }
    ...
    try {
      ...
      if (updateData.email) {
        ...
        if (existing && existing._id?.toString() !== id) {
          res.status(409).json({
            success: false,
            error: 'Cet email est deja utilise par un autre candidat'
          });
          return;
        }
      }
  ...
  delete = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({
        success: true,
        message: 'Candidat desactive avec succes',
        data: candidate
      });
    } catch (error: any) {
      res.status(400).json({
  ...
  getDocuments = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const candidate: any = await this.candidateService.findById(id);
  ...
  updateStatus = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const candidate = await this.candidateService.update(id, { status } as any);
