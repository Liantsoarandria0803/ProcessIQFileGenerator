...
export class AdmissionService {
  ...
  private getDocumentsStatus(fields: CandidatFields): CandidateDocuments {
    const hasDocument = (value: unknown): boolean => {
      if (Array.isArray(value)) return value.length > 0;
      return Boolean(value);
    };

    return {
      cv: hasDocument(fields.CV),
      cin: hasDocument(fields.CIN) || hasDocument(fields['Carte d\'identité']),
      lettre_motivation: hasDocument(fields['lettre de motivation']) || hasDocument(fields['Lettre de motivation']),
      carte_vitale: hasDocument(fields['Photocopie carte vitale']) || hasDocument(fields['Carte vitale']),
      dernier_diplome: hasDocument(fields['dernier diplome']) || hasDocument(fields['Dernier diplôme'])
    };
  }
  ...
  private async uploadDocument(
    ...
    return {
      success: true,
      message: `${candidateFieldName} uploadé avec succès`,
  ...
  async uploadLettreMotivation(recordId: string, file: Express.Multer.File): Promise<UploadResponse> {
    return this.uploadDocument(recordId, file, 'lettre de motivation');
  }
  ...
  async uploadCarteVitale(recordId: string, file: Express.Multer.File): Promise<UploadResponse> {
    return this.uploadDocument(recordId, file, 'Photocopie carte vitale');
  }
  ...
  async uploadDernierDiplome(recordId: string, file: Express.Multer.File): Promise<UploadResponse> {
    return this.uploadDocument(recordId, file, 'dernier diplome');
  }
}
