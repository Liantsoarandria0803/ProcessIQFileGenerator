...
export class FranceCompetencesService {
  private get collection(): mongoose.mongo.Collection<FranceCompetencesRate> {
    const db = mongoose.connection.db;
    if (!db) {
      throw new Error('MongoDB non connecte');
    }
    return db.collection<FranceCompetencesRate>('franceCompetencesRates');
  }
